...
class DenseAtt(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        # 获取源节点和目标节点的特征
        x_left = x[src_nodes]  # [num_edges, d]
        x_right = x[dst_nodes]  # [num_edges, d]
        
        # 拼接特征
        x_cat = torch.cat((x_left, x_right), dim=1)  # [num_edges, 2d]
        
        # 计算注意力权重
        edge_att = self.linear(x_cat).squeeze()
        edge_att = F.sigmoid(edge_att)
        
        # 构建稀疏注意力矩阵
        att_adj = torch.sparse_coo_tensor(
            edge_index,
            edge_att,
            size=(n, n)
        )
    ...
    #     print(f"x_left dtype: {x_left.dtype}")
    #     print(f"x_right dtype: {x_right.dtype}")
