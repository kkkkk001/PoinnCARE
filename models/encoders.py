    ...
    def __init__(self, c, args):
        ...
        if args.concat_agg:
            for i in range(len(dims) - 1):
                act = acts[i]
                if i > 0:
                    gc_layers.append(GraphConvolution(dims[i-1]+dims[i], dims[i + 1], args.dropout, act, args.bias, concat_agg=True))
                else:
                    gc_layers.append(GraphConvolution(dims[i], dims[i + 1], args.dropout, act, args.bias, concat_agg=True))
            gc_layers.append(SelectFirst())
            gc_layers.append(Linear(sum(dims), dims[-1], args.dropout, acts[-1], args.bias))
            gc_layers.append(ReturnTwo())
        else:
            for i in range(len(dims) - 1):
                in_dim, out_dim = dims[i], dims[i + 1]
                act = acts[i]
...
class SelectFirst(nn.Module):
    def __init__(self):
        super(SelectFirst, self).__init__()

    def forward(self, x):
        return x[0]


class ReturnTwo(nn.Module):
    def __init__(self):
        super(ReturnTwo, self).__init__()
